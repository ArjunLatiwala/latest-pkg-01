 ...
 */
exports.setupPreCommitHook = async (gitRoot, projectRoot) => {
  const targetProject = projectRoot || process.cwd();
  const huskyDir = path.join(gitRoot || targetProject, '.husky');
  ...
  const gitleaksIgnorePath = path.join(targetProject, '.gitleaksignore');
