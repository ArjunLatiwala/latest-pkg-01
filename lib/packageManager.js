 ...
 */
exports.installDevDependency = async (pkg, projectRoot = process.cwd()) => {
  const pkgPath = path.join(projectRoot, 'package.json');

  if (!await fs.pathExists(pkgPath)) {
    logInfo(`No package.json found at ${projectRoot}. Skipping dependency install: ${pkg}`);
    return;
  }
  ...
  const isBinaryPresent = await fs.pathExists(path.join(projectRoot, 'node_modules', pkg));
