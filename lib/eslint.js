 ...
 */
exports.setupESLintConfig = async (projectRoot) => {
  const targetProject = projectRoot || process.cwd();

  // Check if TypeScript is used in the project
  const hasTypeScript = await checkForTypeScript(targetProject);
  ...
  if (hasTypeScript) {
    ...
    await installDevDependency('@typescript-eslint/parser', targetProject);
    await installDevDependency('@typescript-eslint/eslint-plugin', targetProject);
    await installDevDependency('typescript', targetProject);
  }
  ...
  for (const file of configFiles) {
    if (await fs.pathExists(path.join(targetProject, file))) {
  ...
  let isLegacy = false;
  try {
    const eslintPkgPath = path.join(targetProject, 'node_modules', 'eslint', 'package.json');
  ...
  const targetPath = path.join(targetProject, templateFile);
